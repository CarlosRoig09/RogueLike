    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        switch (_proyectileUser)
        {
            case ProyectileUser.Player:
                ...
                {
                    collision.gameObject.GetComponent<IDestroyable>().GetHitByPlayer(_damage,0.3f);
                    if (collision.CompareTag("Enemy"))
                        PushOtherGO(collision, _shootSO.ImpulseForce);
                }
                if (!(collision.CompareTag("Enemy")&&collision.isTrigger)&&!collision.CompareTag("Player")&&!collision.CompareTag("Proyectile")&&!collision.CompareTag("Item")&&!collision.CompareTag("Weapon")&&!collision.CompareTag("wotah"))
                    DestroyProyectile();
                break;
