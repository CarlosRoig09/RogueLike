...
public class AxeController : MonoBehaviour, IWeaponControler
{
    ...
    public void SecondButtonAttack()
    {
        ...
        {
            AudioManager.instance.Play("HeavyWeapon");
            CollisionEnable();
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (enabled)
        {
            if (collision.gameObject.GetComponent<IDestroyable>() != null&&!collision.isTrigger)
                collision.gameObject.GetComponent<IDestroyable>().GetHitByPlayer(_weaponSO.WA == WeaponState.MeleeAttack ? WeaponDamage : ProyectileDamage,0.3f);
